  ...
  methods: {
    ...
    async generarVoto(votante, voteType, collectionName) {
      ...
      if (votante.comentario)
        ...
        try {
          const response = await fetch(`http://valley-dojo.netlify.app/api/data/${collectionName}`, {
            method: 'POST',
            headers: {
    ...
    async resetDatabases() {
      ...
      this.loadReset = true
      try {
        const response = await fetch('http://valley-dojo.netlify.app/api/reset-databases', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
